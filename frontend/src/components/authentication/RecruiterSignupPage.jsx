...
const validationSchema = object({
  firstname: z.string().min(1, { message: "Firstname is required." }).refine(
    (value) => !/\d/.test(value),
  ...
  lastname: z.string().min(1, { message: "Lastname is required." }).refine(
    (value) => !/\d/.test(value),
...
const [errorMessage, setErrorMessage] = useState(null); 
  ...
  const onRegisterRequest = async (data) => {
    try{
      setLoading(true);
      await recruiterSignupProvider(data.firstname, data.lastname, data.email, data.password,"recruiter", setErrorMessage, setLoading); reset({
        firstname: "",
        lastname: "",
        email: "",
        password: "",
  ...
  return (
    <Container maxWidth="md">
      ...
      >
        ...
        {errorMessage && (
          <Box sx={{ textAlign: "center", color: "red", fontSize: "20px" , fontFamily: "serif", m: 1}}>
            {errorMessage}
          </Box>
        )}
       {loading ? ( 
